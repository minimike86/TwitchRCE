...
async def check_valid_token(user: any) -> bool:
    is_valid_token = await twitch_api_auth_http.validate_token(
...
async def refresh_user_token(user: any) -> str:
    auth_result = await twitch_api_auth_http.refresh_access_token(
...
async def event_eventsub_notification_stream_start(
    ...
    broadcaster = list(
        filter(lambda x: x.id == payload.data.broadcaster.id, bot.channel_broadcasters)
    )[0]
